         ...
         render={({ field }) => (
            ...
            >
               ...
               <span className="flex items-center gap-1.5 flex-wrap">
                  {label}
                  {isLocked ? (
                     <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.2 rounded-full inline-flex items-center gap-1">
...
export function RiskFactorAssessmentForm({
   ...
   const { data: allDiseases } = useGetAllChronicDiseasesQuery(
      { page: 1, limit: 1000 },
      { skip: !selectedProfile?.id },
   );
   ...
   const lockedDiseaseKeys = useMemo(() => {
      ...
      // 3. Từ selectedProfile.profileChronicDisease.diseaseIds kết hợp allDiseases
      if (
         selectedProfile.profileChronicDisease?.diseaseIds &&
         allDiseases?.items
      ) {
         const idSet = new Set(
            selectedProfile.profileChronicDisease.diseaseIds,
         );
         allDiseases.items.forEach((cd) => {
            if (idSet.has(cd.id)) {
               const matched = matchDiseaseToField(cd);
               if (matched) keys.add(matched);
            }
      ...
      // 4. Từ các trường yếu tố nguy cơ trong hồ sơ sức khỏe mới
      if (selectedProfile.hasDiabetes) {
         keys.add("diabetes");
      }

      return keys;
   }, [selectedProfile, schemaData, profileChronicDiseases, allDiseases]);

   const hasRecordedUnderlying =
      Boolean(schemaData?.patientProfile?.hasRecordedUnderlyingDiseases) ||
      lockedDiseaseKeys.size > 0 ||
      Boolean(selectedProfile?.profileChronicDisease?.diseaseIds?.length) ||
   ...
   const renderSourceBadge = (fieldName: keyof AssessmentFormValues) => {
      ...
      if (source === "PROFILE") {
         return (
            <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.2 rounded-full inline-flex items-center gap-1 shrink-0">
               <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
               Hồ sơ
            </span>
         );
      ...
      if (source === "OCR") {
         return (
            <span className="text-[10px] font-semibold text-violet-700 bg-violet-50 border border-violet-200 px-1.5 py-0.2 rounded-full inline-flex items-center gap-1 shrink-0">
               <Sparkles className="w-2.5 h-2.5 text-violet-600 inline-block" />
               Từ OCR
            </span>
         );
   ...
   return (
      <div className="flex flex-col gap-4 px-0.5">
                  ...
                  assessmentResult.redFlags.length > 0 && (
                     <div className="pt-3">
                        <div className="text-xs font-bold uppercase tracking-wider text-rose-700 mb-2">
                           Dấu hiệu cảnh báo nguy cơ cao (Red Flags):
                        </div>
                        <div className="">
         ...
         <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            {/* Chú thích nguồn dữ liệu ghi nhận */}
                           ...
                           <Controller
                              ...
                              render={({ field }) => (
                                 <FormSelect
                                    ...
                                    value={field.value || undefined}
                                    onValueChange={field.onChange}
                                    placeholder="Chọn mức kiểm soát"
